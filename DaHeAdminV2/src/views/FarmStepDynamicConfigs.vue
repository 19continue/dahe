<template>
  <div class="farm-step-dynamic-config-page">
    <PageToolbar
      :title="pageTitle"
...
const FARM_MODULE_KEY = 'farm'
const FARM_SCENE_KEY = 'step_fields'
...
const scopeLabel = computed(() => '农事步骤参数模板')
const pageTitle = computed(() => '农事步骤参数模板中心')
const pageSubtitle = computed(() => '管理员仅需维护步骤模板字段，不再接触模块和场景技术参数。')
...
async function updateStatusById(id, nextStatus) {
  const detail = await request.get(`/admin/dynamic-configs/${id}`)
  const payload = {
    moduleKey: FARM_MODULE_KEY,
    sceneKey: FARM_SCENE_KEY,
  ...
  await request.put(`/admin/dynamic-configs/${id}`, payload)
}
...
async function removeRow(row) {
  ...
  try {
    await request.delete(`/admin/dynamic-configs/${id}`)
...
async function batchDeleteRows() {
  ...
  try {
    const results = await Promise.allSettled(
      targets.map((row) => request.delete(`/admin/dynamic-configs/${row.id}`))
    )
    const successCount = results.filter((item) => item.status === 'fulfilled').length
    const failedCount = results.length - successCount
...
async function openEdit(row) {
  ...
  editMode.value = true
  try {
    const detail = await request.get(`/admin/dynamic-configs/${row.id}`)
    fillForm(detail || row)
    editorMode.value = 'visual'
    dialogVisible.value = true
  } catch (error) {
...
async function saveConfig() {
  ...
  try {
    ...
    if (editMode.value && form.id) {
      await request.put(`/admin/dynamic-configs/${form.id}`, payload)
      ElMessage.success('配置已更新')
    } else {
      await request.post('/admin/dynamic-configs', payload)
...
async function refreshSelected() {
  ...
  try {
    selected.value = await request.get(`/admin/dynamic-configs/${selected.value.id}`)
  } catch (error) {
